...
function getVercelEnv() {
  try {
    ...
    // Parse the table output: find lines with KEY VALUE [ENV] columns
    const lines = output.split('\n');
    const result = {};
    let headerFound = false;
    for (const line of lines) {
      // Find the header row
      if (!headerFound && /Name\s+Value/i.test(line)) {
        headerFound = true;
        continue;
      }
      ...
      if (!line.trim()) break;
      // Parse columns
      const match = line.match(/^(\S+)\s+(.+?)\s+(Preview|Development|Production|All)$/);
      if (match) {
        const key = match[1];
...
function getSupabaseEnv() {
  try {
    const output = execSync('supabase secrets list -o json', { encoding: 'utf-8' });
    // Output: { secrets: [{ name, value }] } or { secrets: [] } or possibly undefined
    const parsed = JSON.parse(output);
    const secrets = parsed && Array.isArray(parsed.secrets) ? parsed.secrets : [];
    const result = {};
    secrets.forEach(s => {
