...
export const users = pgTable("users", {
  ...
  role: varchar("role"), // landlord, tenant, or property_manager - nullable for new OAuth users
...
// ============================================================================
// PAYMENT RECONCILIATION SYSTEM TABLES
// ============================================================================

// Landlord Payment Channels - Registry of how landlords receive money
...
// Invoices - Formalized bills/receivables generated by system
...
// External Payment Events - Raw immutable webhook events
...
export const landlordPaymentChannelsRelations = relations(landlordPaymentChannels, ({ one, many }) => ({
  landlord: one(users, {
    fields: [landlordPaymentChannels.landlordId],
    references: [users.id],
  }),
  paymentEvents: many(externalPaymentEvents),
}));
...
export const externalPaymentEventsRelations = relations(externalPaymentEvents, ({ one }) => ({
  landlord: one(users, {
    fields: [externalPaymentEvents.landlordId],
    references: [users.id],
  }),
  paymentChannel: one(landlordPaymentChannels, {
    fields: [externalPaymentEvents.paymentChannelId],
    references: [landlordPaymentChannels.id],
  }),
}));
...
export const insertLandlordPaymentChannelSchema = createInsertSchema(landlordPaymentChannels).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertExternalPaymentEventSchema = createInsertSchema(externalPaymentEvents).omit({
  id: true,
  createdAt: true,
  receivedAt: true,
});
...
export type LandlordPaymentChannel = typeof landlordPaymentChannels.$inferSelect;
export type InsertLandlordPaymentChannel = z.infer<typeof insertLandlordPaymentChannelSchema>;
export type Invoice = typeof invoices.$inferSelect;
export type InsertInvoice = z.infer<typeof insertInvoiceSchema>;
export type ExternalPaymentEvent = typeof externalPaymentEvents.$inferSelect;
export type InsertExternalPaymentEvent = z.infer<typeof insertExternalPaymentEventSchema>;
