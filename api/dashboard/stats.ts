...
export default requireAuth(async (req: VercelRequest, res: VercelResponse, auth) => {
  ...
  try {
    ...
    const [
      ...
      `,
      // 7. Get revenue trend (last 6 months)
      sql`
        SELECT 
        ...
        ORDER BY month ASC
      `,
      // 8. Get recent completed payments (last 5)
      sql`
        SELECT 
          pm.*,
    ...
    const totalPayments = parseInt(transactionsCount?.[0]?.count || '0'); // Corrected variable name below in assignment
